...
export default class Board extends React.Component {
  ...
  componentDidMount() {
    ...
    this.drake.on("drop", (el, target, source, sibling) => {
      ...
      if (newStatus !== oldStatus) {
        this.moveClient(clientId, oldStatus, newStatus);
      }
    });
  }
  ...
  moveClient(clientId, fromStatus, toStatus) {
    this.setState((prevState) => {
      const clientToMove = prevState.clients[fromStatus].find(
        (client) => client.id === clientId
      );
      const updatedClients = {
        ...prevState.clients,
        [fromStatus]: prevState.clients[fromStatus].filter(
          (client) => client.id !== clientId
        ),
        [toStatus]: [
          ...prevState.clients[toStatus],
          { ...clientToMove, status: toStatus },
        ],
      };
